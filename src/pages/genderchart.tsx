...
const GenderChart = (props: { isDarkMode: boolean }) => {
  ...
  onMount(async () => {
    try {
      const response = await fetch("http://127.0.0.1:8080/genderchart");
      if (!response.ok) throw new Error("Network response was not ok");
      const genderData: GenderData = await response.json();

      // Prepare data for the chart
      const data = [
        { name: "Laki-laki", value: genderData.laki_laki || 0 },
        { name: "Perempuan", value: genderData.perempuan || 0 },
      ];
  ...
  return <div ref={chartDiv} id="chartdiv" style={{ "margin-top": "-25px", "margin-left": "17px", width: "350px", height: "200px" }}></div>;
};
